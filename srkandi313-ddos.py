...
time_loader = time.time() + int(input(f"{Fore.GREEN}::TIME ==⟩⟩ {Fore.LIGHTYELLOW_EX} "))
spam_loader = int(input(f"{Fore.BLUE}::SPAM THREAD ==⟩⟩ {Fore.LIGHTYELLOW_EX} "))
create_thread = int(input(F"{Fore.CYAN}::CREATE THREAD ==⟩⟩ {Fore.LIGHTYELLOW_EX}"))
booter_sent = int(input(F"{Fore.GREEN}::BOOTER SENT ==⟩⟩ {Fore.LIGHTYELLOW_EX} "))
